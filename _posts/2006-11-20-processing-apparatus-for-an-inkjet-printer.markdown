...
---
This is a Continuation of Ser. No. 11 003 362 filed on Dec. 6 2004 now U.S. Pat. No. 7 164 505 which is a Continuation of Ser. No. 10 636 229 filed on Aug. 8 2003 now issued as U.S. Pat. No. 7 070 251 which is a Divisional of Ser. No. 09 436 747 now issued as U.S. Pat. No. 6 687 022 all of which are herein incorporated by reference.
...
In one broad form the invention comprises a method of controlling the image printed by a printhead having nozzles which discharge a dot of ink by means of a firing pulse generated by an actuator said method comprising the steps of 
...
Table 7 shows Group 4 and EDRL compression performance on pages of text of varying point sizes rendered at 800 dpi. Note that EDRL achieves the required compression ratio of 2.5 for an entire page of text typeset at a point of 3. The distribution of characters on the test pages is based on English language statistics 13 .
...
The state machine in turn outputs the length of the code. This is added modulo 8 to the current codeword bit offset to yield the next codeword bit offset. The bit offset in turn controls the barrel shifter . If the codeword bit offset wraps then the carry bit controls the latching of the next byte from the input FIFO . At this time byte is latched to byte and the FIFO output is latched to byte . It takes two cycles of length to input buffer. This is handled by starting states in the state machine .
...
Each buffer has its own WriteEnable line to allow a single 32 bit value to be written to a particular color buffer in a given cycle. The 32 bits of DataIn are shared since only one buffer will actually clock the data in.
